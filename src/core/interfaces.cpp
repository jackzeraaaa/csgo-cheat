...
void interfaces::Setup() noexcept
{
	...
	trace = Capture<IEngineTraceClient>("engine.dll", "EngineTraceClient004");
	...
	cvar = Capture<EngineConVar>("vstdlib.dll", "VEngineCvar");
	panel = Capture<Panel>("vgui2.dll", "VGUI_Panel009");
	surface = Capture<ISurface>("vguimatsurface.dll", "VGUI_Surface031");

	//input = *(CInput**)((*(DWORD**)client)[15] + 0x1);
	input = *reinterpret_cast<CInput**>((*reinterpret_cast<uintptr_t**>(client))[16] + 1);
